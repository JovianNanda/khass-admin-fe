<script setup lang="ts">
import { reactive, ref, watch } from 'vue'
import { useForm } from 'vuestic-ui/web-components'
import { useTheaterStore } from '../../stores/theaters'
import { storeToRefs } from 'pinia'
import router from '../../router'
import { useBundleStore } from '../../stores/bundles'
import { formatDate } from '../../services/utils'
import { formBundle } from '../../types/bundle'
import { selectOption } from '../../types/helpers'
import { onBeforeRouteLeave } from 'vue-router'

const { getAllTheaters } = useTheaterStore()
const { theaters, isLoading } = storeToRefs(useTheaterStore())
const { isValid, validate } = useForm('formRef')

const { createNewBundle } = useBundleStore()

const { today, formatDataDate, formatDateFn, parseDateFn, formatDataTime } = formatDate

const defaultValue: formBundle = {
  name: '',
  kuota: 0,
  theater: null as unknown as selectOption[],
  showDate: today,
  // closeDate: closeDateDefault(),
  closeDate: today,
  closeTime: `0000`,
  desc: '',
  price: 0,
  visibility: true,
  image_url: '',
  show_type: '',
}

const newBundle: formBundle = reactive({ ...defaultValue })

const theaterOptions = ref<selectOption[]>([])

const goToAllBundle = () => {
  router.push({ name: 'bundle-list' })
}

const isOnSave = ref(false)

const onSave = () => {
  if (validate()) {
    isOnSave.value = true
    // console.log('old', newBundle);
    if (!newBundle.theater) return
    const cleanedTheater = newBundle.theater.filter((theater) => theater !== null) //remove null value
    const theaterData: number[] = cleanedTheater.map((theater: selectOption) =>
      typeof theater.value === 'string' ? parseInt(theater.value) : theater.value,
    )

    createNewBundle({
      name: newBundle.name,
      theaters: theaterData,
      kuota: newBundle.kuota,
      date: formatDataDate(newBundle.showDate),
      description: newBundle.desc,
      price: newBundle.price,
      close_at: `${formatDataDate(newBundle.closeDate)} ${formatDataTime(newBundle.closeTime)}`,
      visibility: newBundle.visibility ? 1 : 0,
      image_url: newBundle.image_url,
      show_type: newBundle.show_type,
    })
  }
}

watch(theaters, (newTheaters) => {
  theaterOptions.value = newTheaters.map((theater) => ({
    text: theater.name,
    value: theater.id,
  }))
})

const isFormDirty = () => {
  return JSON.stringify(newBundle) !== JSON.stringify(defaultValue)
}

onBeforeRouteLeave(() => {
  if (isFormDirty() && !isOnSave.value) {
    return window.confirm('Do you really want to leave? You have unsaved changes!')
  }
})

getAllTheaters()
</script>

<template>
  <div class="flex flex-col sm:flex-row gap-2 mb-4 justify-between">
    <h1 class="page-title">Create Bundle</h1>
    <div class="grid content-center">
      <VaButton icon="arrow_back" @click="goToAllBundle">Back</VaButton>
    </div>
  </div>

  <VaCard>
    <VaCardContent>
      <VaForm ref="formRef" class="flex flex-col items-baseline gap-6">
        <VaInput
          v-model="newBundle.name"
          :rules="[(value) => (value && value.length > 0) || 'name is required']"
          label="Bundle Name"
        />
        <VaInput
          v-model="newBundle.show_type"
          :rules="[(value) => (value && value.length > 0) || 'show type is required']"
          label="Show Type"
        />

        <VaSelect
          v-model="newBundle.theater"
          :loading="isLoading"
          :options="theaterOptions"
          multiple
          :rules="[(v) => v || 'Field is required']"
          label="Theater"
        />

        <VaInput
          v-model="newBundle.image_url"
          :rules="[(value) => (value && value.length > 0) || 'image is required']"
          label="Image URL"
        />

        <div class="flex flex-wrap gap-4">
          <VaInput
            v-model="newBundle.kuota"
            style="width: 150px"
            label="Kuota"
            inputmode="numeric"
            mask="numeral"
            placeholder="####"
            :rules="[(v) => v.length > 0 || 'Kuota is required', (v) => parseInt(v) > 0 || 'Kuota can\'t less than 1']"
          >
            <template #appendInner>
              <span>Orang</span>
            </template>
          </VaInput>
          <VaInput
            v-model="newBundle.price"
            style="width: 200px"
            label="Price"
            inputmode="numeric"
            mask="numeral"
            placeholder="####"
            :rules="[(v) => v.length > 0 || 'Price is required', (v) => parseInt(v) > 0 || 'Price can\'t less than 1']"
          >
            <template #prependInner>
              <span>Rp.</span>
            </template>
          </VaInput>
        </div>

        <div class="flex flex-wrap gap-4 align-end">
          <VaDateInput
            v-model="newBundle.showDate"
            :format="formatDateFn"
            :parse="parseDateFn"
            clearable
            label="The Show Date"
          />
          <VaCheckbox v-model="newBundle.visibility" label="Bundle Visibility" class="mt-5" />
        </div>
        <div class="flex flex-wrap gap-4">
          <VaDateInput
            v-model="newBundle.closeDate"
            :format="formatDateFn"
            :parse="parseDateFn"
            clearable
            label="Close Registration Date"
          />
          <!-- <VaInput v-model="newBundle.closeDate" style="width: fit-content;" label="Close Registration Date"
            placeholder=" DD/MM/YYYY" mask="date" messages="DD/MM/YYYY" /> -->
          <VaInput
            v-model="newBundle.closeTime"
            style="width: 150px"
            label="Close Registration Time"
            placeholder="HH:MM"
            mask="time"
            :messages="['HH:MM', 'Hours:minutes']"
            :rules="[(v) => v.length === 4 || 'Time not valid']"
          />
        </div>
        <VaTextarea v-model="newBundle.desc" style="width: 100%" label="Bundle Description" />
      </VaForm>

      <VaButton :disabled="!isValid" class="mt-3" @click="onSave"> Create </VaButton>
    </VaCardContent>
  </VaCard>
</template>
