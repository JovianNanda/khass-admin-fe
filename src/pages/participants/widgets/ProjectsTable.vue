...
const columns = defineVaDataTableColumns([
  ...
  { label: 'transaction_status', key: 'transaction_status' },
  { label: 'name', key: 'name' },
...
defineEmits<{
  (event: 'view', participant: Participant): void
}>()
    ...
    >
      <template #cell(transaction_status)="{ rowData }">
        <div class="flex items-center gap-2 ellipsis max-w-[230px]">
          <VaBadge
            :text="rowData.transaction_status"
            :color="transactionStatusBadgeColor(rowData.transaction_status)"
            class="mr-2"
          />
        </div>
      </template>
      <template #cell(fee)="{ rowData }">
        <div class="flex items-center gap-2 ellipsis max-w-[230px]">
          <span class="text-sm">{{ rupiah(rowData.fee) }}</span>
        </div>
      </template>
      ...
      <template #cell(actions)="{ rowData: participant }">
        <div class="flex gap-2 justify-end">
          <VaButton
            ...
            @click="$emit('view', participant as Participant)"
          />
        </div>
      </template>
